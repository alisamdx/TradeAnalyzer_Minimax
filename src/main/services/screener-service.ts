...
export const DEFAULT_FILTER_SPECS: FilterSpec[] = [
  ...
  { id: 'eps_growth',       label: 'EPS Growth YoY',    enabled: true,  defaultMin: 0,             defaultMax: Infinity,       defaultEnabled: true,  format: 'percent',  description: '≥ 0%: earnings are not shrinking' },
  ...
  { id: 'avg_volume',       label: 'Avg Daily Volume',  enabled: true,  defaultMin: 500_000,    defaultMax: Infinity,       defaultEnabled: true,  format: 'count',    description: '≥ 500K shares: basic liquidity' },
  ...
  { id: 'dist_52wk_high',   label: 'Dist. 52-wk High', enabled: false,  defaultMin: 0,             defaultMax: 25,             defaultEnabled: false,  format: 'percent',  description: 'Within 25%: near highs' },
  { id: 'dist_52wk_low',    label: 'Dist. 52-wk Low',  enabled: false,  defaultMin: 15,            defaultMax: Infinity,       defaultEnabled: false,  format: 'percent',  description: '≥ 15%: off the absolute bottom' },
    ...
    for (const fd of criteria.filters) {
      ...
      switch (fd.id) {
        ...
        case 'dist_52wk_high': {
          ...
          // Pass if data unavailable, otherwise check range
          check(fd.id, dist === null || (dist >= mn && dist <= mx));
          break;
        }
        case 'dist_52wk_low': {
          const mn = (v['min'] as number) ?? 0;
          const dist = quote?.distance52WkLow ?? null;
          // Pass if data unavailable, otherwise check min distance from low
          check(fd.id, dist === null || dist >= mn);
          break;
        }
        case 'sector_exclude': {
