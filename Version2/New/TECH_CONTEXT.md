...
#### C. Cache Manager Service (Auto-Refresh)
```
App Start
    │
    ▼
[Backend] ├─→ Check cache_metadata.last_screener_run
    │      ├─→ If > 1 hour: emit "cache-stale" event
    │      └─→ Frontend receives event → trigger auto-refresh
    │
    ▼
[Frontend] ├─→ useCacheStatus hook polls every 5 minutes
    │       └─→ Updates CacheStatusIndicator component
    │
    ▼
[User]    ├─→ Sees visual indicator (Green=Fresh, Red=Stale)
          └─→ Manual refresh via button in Screener/Watchlist
```
...
CREATE TABLE cache_metadata (
    ...
    last_screener_run INTEGER,  -- Unix timestamp
    record_count INTEGER,       -- Cached ticker count
    updated_at INTEGER
);
```

**Benefits**: Automatic data freshness, user sees staleness at a glance, manual override available.
...
-- Cache tracking
cache_metadata      -- Last screener run, record counts
...
#### useSortable
Generic sorting hook for table columns with asc/desc toggle:
```typescript
const { sortedData, sortConfig, requestSort } = useSortable(
  data,
  'roe',      // default sort key
  'desc'      // default direction
);
```
- Handles null values by pushing to end
- Returns sort indicator for UI arrows
- Used in: ScreenerView, WatchlistView, MorningBriefingView

#### useCacheStatus
Polls cache freshness every 5 minutes:
```typescript
const { status, isStale, lastUpdated, refresh } = useCacheStatus();
```
- Triggers auto-refresh callback when stale
- Formats age as human-readable string (e.g., "45 min ago")
- Integrates with CacheStatusIndicator component
...
├── Sidebar (navigation)
├── WatchlistView
│   ├── CacheStatusIndicator (under watchlist name)
│   ├── WatchlistList
│   ├── WatchlistItems (table with fundamentals + Wheel columns)
│   └── QuoteCards
├── ScreenerView
│   ├── CacheStatusIndicator (in header)
│   ├── FilterPanel
...
│   ├── MarketRegimeCard
│   ├── ActionItemsList
│   └── TopSetups (15 rows with Wheel columns)
└── SettingsView
└── DiagnosticsPanel
```
...
**Wheel Suitability Score** (0-100):
```
Score = ROE*30% + D/E_Quality*30% + MarketCap*20% + Stability*25%

ROE Component: min(ROE/30 * 100, 100)
D/E Component: if D/E < 1.0: 100, else: max(0, 100 - (D/E-1)*50)
Market Cap: if > $50B: 100, > $10B: 75, > $1B: 50, else: 25
Stability: Based on volatility/sector
```

**Target Strike**: Current Price × 0.92 (8% OTM)

**Est. Premium**: Target Strike × 1.2% (30 DTE approximation)

**Note**: Computed in Rust, cached in SQLite, recalculated on fresh fetch.

### Cache Manager Service (Rust)
...
impl CacheManager {
    pub fn is_cache_stale(&self, threshold_hours: i64
    ) -> Result<bool> {
        // Compare last_screener_run against current time
    }

    pub fn get_cache_stats(&self) -> Result<CacheStats> {
        // Return age, record_count, status
    }
...
**Backend Auto-Refresh**: On app start, checks if cache > 1 hour old, emits `cache-stale` event if so.
